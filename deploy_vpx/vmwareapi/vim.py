...
from deploy_vpx.vmwareapi.error_util import FaultCheckers
from deploy_vpx.vmwareapi.error_util import SessionOverLoadException
from deploy_vpx.vmwareapi.error_util import VimAttributeError
from deploy_vpx.vmwareapi.error_util import VimException
from deploy_vpx.vmwareapi.error_util import VimFaultException
...
class Vim:
    ...
    def __init__(self,
        ...
        wsdl_url = FLAGS.WSDL_LOCATION
        if wsdl_url is None:
            raise Exception("Could not find vimService.wsdl.")
            ...
            def vim_request_handler(managed_object, **kwargs):
                ...
                **kwargs          : Keyword arguments of the call
                ...
                try:
                    ...
                    # SOAP server
                    if hasattr(FaultCheckers,
                                    attr_name.lower() + "_fault_checker"):
                        fault_checker = getattr(FaultCheckers,
                                    attr_name.lower() + "_fault_checker")
                ...
                # check of the SOAP response
                    ...
                    raise VimFaultException(fault_list, excep)
                except AttributeError, excep:
                    raise VimAttributeError("No such SOAP method "
                         "'%s' provided by VI SDK" % (attr_name), excep)
                except (httplib.CannotSendRequest,
                        httplib.ResponseNotReady,
                        httplib.CannotSendHeader), excep:
                    raise SessionOverLoadException("httplib "
                    ...
                    if (str(excep).find(ADDRESS_IN_USE_ERROR) != -1 or
                        ...
                        raise SessionOverLoadException(("Socket "
                                    "error in %s: ") % (attr_name), excep)
                    # Type error that needs special handling for it might be
                    # caused by ESX host API call overload
                    elif str(excep).find(RESP_NOT_XML_ERROR) != -1:
                        raise SessionOverLoadException("Type "
                                    "error in  %s: " % (attr_name), excep)
                    else:
                        raise VimException(
